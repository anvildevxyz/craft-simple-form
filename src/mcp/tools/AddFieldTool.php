...
class AddFieldTool implements ToolInterface
{
    ...
    public function call(array $arguments): array
    {
        ...
        // so Solo may not add a Standard field type here — the same rule the CP save
        // enforces, which this non-CP authoring path would otherwise bypass.
        if (!Editions::fieldTypeAllowed($type)) {
            return [
                'isError' => true,
                'errors' => ['type' => [sprintf('The "%s" field type requires the Standard edition.', $type)]],
            ];
        }

        // Same gate for Standard capabilities the field's own config may introduce
        ...
        if ($blockedCaps !== []) {
            return [
                'isError' => true,
                'errors' => ['config' => [sprintf('This field uses features that require the Standard edition: %s.', implode(', ', $blockedCaps))]],
            ];
        }
