...
class FormPortabilityService extends Component
{
    ...
    public function import(array $data, array $opts = []): ImportResult
    {
        ...
        // Edition gate: importing a brand-new form must not slip Standard fields or
        // Standard form-capabilities onto Solo (the CP/console/forms-as-code paths all
    ...
    public function applyToExistingForm(Form $form, array $data, bool $prune, ImportResult $result): void
    {
        ...
        // Edition gate (no-escalation): the file may keep Standard features the form
    ...
    /**
     * Reject an import/apply that would introduce Standard field types or Standard
     ...
     * directly. A Standard feature already present on the target form is allowed
     ...
     * @param array<string, bool> $existingModes the target form's stored Standard modes ([] for a new import)
    ...
    private function assertEditionAllows(array $data, array $existingItems, bool $existingSaveResume, array $existingModes = []): void
    {
        ...
        $blocked = array_merge(
            Editions::blockedNewStandardFields($types, $existingTypes),
        ...
        if ($blocked !== []) {
            throw new InvalidArgumentException(Craft::t(
                'simple-form',
                'This form requires the Standard edition (uses: {items}).',
