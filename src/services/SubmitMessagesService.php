 ...
 *
 * Adding a new row is a Standard capability gated by
...
class SubmitMessagesService extends Component
{
     ...
     * requires the Standard conditional-logic capability; editing an existing row is
    ...
    public function save(SubmitMessageModel $model): bool
    {
        ...
        // Standard capability; an update to an existing row does not.
        if ($isNew && !Editions::can(Editions::CAP_CONDITIONAL_LOGIC)) {
            $model->addError('conditional', Craft::t(
                'simple-form',
                'Conditional submit messages are a Standard feature.',
            ));
            return false;
        }
