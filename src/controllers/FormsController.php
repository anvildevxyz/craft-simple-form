...
class FormsController extends Controller
{
    ...
    public function actionSave(): Response
    {
        ...
        // edition gate can tell newly-enabled Standard capabilities from ones the form
        ...
        // Scalar Standard modes aren't derivable from the field set, so snapshot them
        ...
        // Edition gate (authoritative): Solo may not introduce *new* Standard field
        // types. Standard fields already on the form survive a downgrade and stay
        ...
        $blockedStandardFields = Editions::blockedNewStandardFields(
            array_values($submittedTypes),
            array_values($existingTypes),
        );
        if ($blockedStandardFields) {
            Craft::$app->getSession()->setError(Craft::t(
                'simple-form',
                'These field types require the Standard edition: {types}',
                ['types' => implode(', ', $blockedStandardFields)],
            ));
            return $this->renderEdit($form, $site, $this->encodeBuilderJson($items), $submitMessagesJson);
        }

        // Edition gate (authoritative): Solo may not newly enable Standard form-level
        ...
        if ($blockedCaps) {
            Craft::$app->getSession()->setError(Craft::t(
                'simple-form',
                'These features require the Standard edition: {features}',
        ...
        if (!Editions::isStandard()) {
            ...
            foreach ($messageRows as $messageRow) {
                ...
                if ($id === null || !in_array($id, $existingMessageIds, true)) {
                    Craft::$app->getSession()->setError(Craft::t(
                        'simple-form',
                        'Conditional submit messages require the Standard edition.',
                    ));
                    return $this->renderEdit($form, $site, $this->encodeBuilderJson($items), $submitMessagesJson);
                }
            }
    ...
    private function renderEdit(Form $form, Site $site, string $builderDataJson, ?string $submitMessagesJson = null): Response
    {
        ...
        return $this->renderTemplate('simple-form/forms/edit', [
            ...
            // Standard features this form already uses while running on Solo (a
    ...
    /**
     * The human-facing Standard features an existing form already uses while the
     * active edition is Solo. Empty on Standard, or for a form with no Standard usage.
     *
     * @return list<string>
     */
    private function proFeaturesInUse(Form $form, Site $site): array
    {
        if (Editions::isStandard() || !$form->id) {
            return [];
        }
        ...
        $standardFields = Editions::blockedNewStandardFields(
            array_map(static fn(array $row): string => (string)$row['type'], $fields),
            [],
        );
        if ($standardFields !== []) {
            $features[] = Craft::t('simple-form', 'Standard field types ({types})', ['types' => implode(', ', $standardFields)]);
        }

        // Derive the form-level capabilities from the same enumeration the save
        // gate uses (empty "existing" => every Standard capability currently in use is
    ...
    /**
     * Human-facing label for a gated form capability, for the Standard-required error.
     */
