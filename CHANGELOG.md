...
- Two editions, **Solo** and **Pro**. Solo is the "better contact form":
  ...
  translation. Pro adds the 11 advanced field types (signature, payment,
...
- The edition gate governs **authoring, never the visitor-facing runtime**: a
  ...
  to Solo. Saving preserves its Pro features but cannot extend them, and the
  editor shows a non-blocking "Pro features in use" banner. Spam protection,
  denylists, and retention keep running after a downgrade; the Pro-only
  back-office services (conditional submit-message resolution, PDF attachments,
  audit logging) pause gracefully and resume on returning to Pro.
...
- Simple Form is commercial software — see [LICENSE.md](LICENSE.md).
