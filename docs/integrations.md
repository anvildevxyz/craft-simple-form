...
> other connector requires [Standard](editions.md). An integration configured on Standard
> keeps dispatching after a downgrade — you just can't add another of a Standard type.
...
| **Slack** | `slack` | Standard | Post a submission message to a Slack channel via incoming webhook. |
| **Discord** | `discord` | Standard | Post a submission message to a Discord channel via webhook. |
| **Mailchimp** | `mailchimp` | Standard | Subscribe the submitter to a Mailchimp audience. |
| **ActiveCampaign** | `activecampaign` | Standard | Create/update a contact in ActiveCampaign. |
| **HubSpot** | `hubspot` | Standard | Create/update a contact in HubSpot. |
| **Pipedrive** | `pipedrive` | Standard | Create a person/lead in Pipedrive. |
| **Google Sheets** | `google-sheets` | Standard | Append one row per submission to a worksheet ([details](#google-sheets)). |
