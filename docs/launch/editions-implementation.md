...
- **P6 — downgrade UX:** DONE. `FormsController::proFeaturesInUse()` computes the Pro features an existing form already uses while on Solo (Pro fields, conditional logic, multi-page, save & continue); the form editor shows a non-blocking "Pro features in use" warning banner listing them. The no-new-escalation save rule (P1/P3) means those features are preserved on save but can't be extended.
- **P7 — tests, docs, store config:** DONE (code/tests/docs). Both-edition completeness smoke test (`tests/smoke/EditionMatrixCest.php`) green; the test harness pins Pro via `Helper\Integration` so both suites cover Pro while the dedicated edition tests flip to Solo; CHANGELOG + README edition table added. **Remaining manual step (not code):** configure the Solo ($19) and Pro ($79) editions + prices in the Plugin Store developer console before listing, and set the DDEV dev site's edition explicitly for CP Playwright smoke work.
