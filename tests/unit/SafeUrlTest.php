...
class SafeUrlTest extends TestCase
{
    ...
    public static function blockedUrls(): array
    {
        return [
            ...
            // IPv4-mapped / -compatible / NAT64 IPv6 spellings of internal targets:
            // PHP's private/reserved filter does not flag these, but the kernel
            // routes them to the real internal IPv4 (SSRF bypass). Must be blocked.
            'ipv4-mapped imds' => ['http://[::ffff:169.254.169.254]/latest/meta-data/'],
            'ipv4-mapped loopback' => ['http://[::ffff:127.0.0.1]/'],
            'ipv4-mapped loopback hex' => ['http://[::ffff:7f00:1]/'],
            'ipv4-mapped private 10/8' => ['http://[::ffff:10.0.0.5]/'],
    ...
    public static function allowedUrls(): array
    {
        return [
            ...
            // A genuinely public address in mapped form stays allowed — the guard
            // unwraps then range-checks, it does not blanket-block mapped IPv6.
            'ipv4-mapped public' => ['http://[::ffff:8.8.8.8]/hook'],
        ];
    }
